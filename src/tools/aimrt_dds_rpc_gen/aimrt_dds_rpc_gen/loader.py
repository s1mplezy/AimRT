...
def _validate(unit: TranslationUnit) -> None:
    ...
    for interface in unit.interfaces:
        if interface.inherited:
            raise IdlDiagnostic(
                E_INHERITANCE,
                f"interface inheritance is not supported: {
                    interface.fqn}",
                interface.inherited[0].location)
        ...
        for operation in interface.operations:
            ...
            if _resolve_struct(operation.response_type, interface.scope, structs) is None:
                raise IdlDiagnostic(
                    E_UNKNOWN_TYPE,
                    f"response type must resolve uniquely to a struct: {
                        operation.response_type.spelling}",
                    operation.response_type.location)
            if _resolve_struct(parameter.type_ref, interface.scope, structs) is None:
                raise IdlDiagnostic(
                    E_UNKNOWN_TYPE,
                    f"request type must resolve uniquely to a struct: {
                        parameter.type_ref.spelling}",
