...
namespace aimrt::examples::cpp::dds_chn::loaned_benchmark_publisher_module {
namespace {
  ...
  try {
    ...
    AIMRT_CHECK_ERROR_THROW(
        aimrt::channel::RegisterPublishType<
            aimrt_examples::dds::BenchmarkSignal>(signal_publisher_),
...
void LoanedBenchmarkPublisherModule::StartMultiTopicPlan(
    uint32_t plan_id, const BenchPlan& plan) {
  aimrt_examples::dds::BenchmarkSignal begin;
  ...
  std::this_thread::sleep_for(std::chrono::seconds(1));
  aimrt_examples::dds::BenchmarkSignal end;
  end.status(aimrt_examples::dds::BenchmarkStatus::END);
