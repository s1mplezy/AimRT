...
bool LoanedBenchmarkSubscriberModule::Initialize(aimrt::CoreRef core) {
  ...
  try {
    ...
    AIMRT_CHECK_ERROR_THROW(
        aimrt::channel::Subscribe<example_ros2::msg::BenchmarkSignal>(
            signal_subscriber_,
...
void LoanedBenchmarkSubscriberModule::SignalHandle(
  ...
  try {
    ...
    if (signal->status == example_ros2::msg::BenchmarkStatus::BEGIN) {
    ...
    } else if (signal->status == example_ros2::msg::BenchmarkStatus::END) {
      {
        std::lock_guard lock(state_mutex_);
        if (has_completed_plan_ &&
            signal->bench_plan_id <= last_completed_plan_id_)
